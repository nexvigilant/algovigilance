...
async function checkBuilders(): Promise<HealthCheckResult> {
  try {
    ...
    const requiredBuilders = ['page-builder.tsx', 'form-builder.ts'];
...
async function checkCourseValidation(): Promise<HealthCheckResult> {
  try {
    ...
    const cliScript = path.join(process.cwd(), 'scripts', 'diagnostics', 'validate-course.ts');
    if (!fs.existsSync(cliScript)) {
      missingFiles.push('scripts/diagnostics/validate-course.ts');
    }
