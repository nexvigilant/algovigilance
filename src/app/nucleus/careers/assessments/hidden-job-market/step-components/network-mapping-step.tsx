...
import { Network, ChevronRight, Lightbulb, CheckCircle2, Target } from 'lucide-react';
...
export function NetworkMappingStep({ responses, onUpdate, onNext }: NetworkMappingStepProps) {
  const section = NAVIGATOR_SECTIONS.find(s => s.id === 'network-mapping') ?? NAVIGATOR_SECTIONS[0];
  const instructions = STEP_INSTRUCTIONS['network-mapping'];
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start gap-4">
        <div className="p-3 bg-cyan/10 rounded-xl">
          <Network className="h-8 w-8 text-cyan" />
        </div>
                ...
                {prompt.pvContext && (
                  <div className="bg-purple-500/5 border border-purple-500/20 rounded-lg p-3">
                    <p className="text-xs text-muted-foreground">
                      <span className="font-semibold text-purple-400">PV Context: </span>
                      {prompt.pvContext}
                    </p>
                  </div>
                )}
      ...
      <div className="flex justify-end pt-4">
        ...
        >
          Continue to Visibility Building
