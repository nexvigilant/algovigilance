...
const log = logger.scope("community/discover/matches");

export default function DiscoverMatchesError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    log.error("Discovery matches error boundary caught:", error);
    reportError(error, {
      component: "DiscoverMatchesError",
      route: "/nucleus/community/discover/matches",
      digest: error.digest,
    });
  }, [error]);
