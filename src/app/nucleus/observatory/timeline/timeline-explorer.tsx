...
const TIMELINE_LEGEND: { label: string; color: string }[] = [
  ...
  { label: 'Regulatory',    color: '#f97316' },
  { label: 'Clinical',      color: '#06b6d4' },
  { label: 'Manufacturing', color: '#22c55e' },
  { label: 'Post-market',   color: '#a855f7' },
  { label: 'Year Marker',   color: '#475569' },
]
...
export function TimelineExplorer() {
  ...
  const { data, loading, error, refetch } = useTimelineData(drug)
    ...
    <div className="mx-auto max-w-7xl px-4 py-golden-4">
      ...
      <header className="mb-golden-4">
        <div className="flex items-center gap-golden-2 mb-golden-2">
          ...
          <div className="flex h-8 w-8 shrink-0 items-center justify-center border border-sky-500/30 bg-sky-500/5">
            <Clock className="h-4 w-4 text-sky-400" aria-hidden="true" />
          </div>
          <div>
            <p className="text-[9px] font-mono uppercase tracking-[0.2em] text-sky-400/70">{STRINGS.brandSubtitle}</p>
            <h1 className="text-sm font-semibold text-white">Timeline</h1>
          </div>
        </div>
      </header>

      <ExplorerNav current="timeline" />

      {/* ── Drug Input ──────────────────────────────────────────────── */}
      <div className="mb-golden-3">
        <label htmlFor="timeline-drug-input" className="text-[10px] font-mono uppercase tracking-wider text-slate-dim/70 block mb-golden-1">
          Drug Name
        </label>
        <input
          id="timeline-drug-input"
          type="text"
          value={drug}
          onChange={e => setDrug(e.target.value)}
          placeholder="e.g. metformin, atorvastatin..."
          className="w-full max-w-xs bg-nex-surface border border-sky-500/30 px-3 py-1.5 text-sm text-white placeholder:text-slate-dim/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-400"
        />
        <p className="text-[10px] text-slate-dim/50 mt-1 font-mono">
          {drug.length < 2 ? 'Demo data shown — enter 2+ characters to query live signals' : `Querying live signal data for "${drug}"`}
        </p>
      </div>
      ...
      {loading && (
        <div className="flex items-center gap-golden-2 mb-golden-3">
          <Loader2 className="h-4 w-4 text-sky-400 animate-spin" aria-hidden="true" />
          <span className="text-sm text-slate-dim/70">Loading timeline data...</span>
        </div>
      )}
      {error && (
        <div role="alert" className="flex items-center gap-golden-2 mb-golden-3 p-golden-2 border border-red-500/30 bg-red-500/5">
          ...
          <button
            onClick={refetch}
            className="text-sm text-cyan hover:text-white transition-colors ml-auto focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-400"
          >
            Retry
          </button>
        </div>
      ...
      {data && (
        <>
          ...
          <div className="flex items-center gap-golden-1 mb-golden-2">
            ...
            {(['force', 'chronological', 'radial'] as const).map((mode) => (
              <button
                key={mode}
                onClick={() => setGraphLayout(mode === 'chronological' ? 'hierarchy' : mode as GraphLayout)}
                aria-pressed={
                  mode === 'chronological' ? graphLayout === 'hierarchy' : graphLayout === mode
                }
                className={`px-2.5 py-1 text-xs font-mono transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-400 ${
                  (mode === 'chronological' ? graphLayout === 'hierarchy' : graphLayout === mode)
                    ? 'bg-sky-500/20 text-sky-400 border border-sky-500/40'
                    : 'bg-nex-surface text-slate-dim/50 border border-nex-light hover:border-sky-500/30 hover:text-white'
                }`}
              >
                {mode === 'force' ? 'Force' : mode === 'chronological' ? 'Chronological' : 'Radial'}
              </button>
            ))}
          </div>
          ...
          <div style={{ height: LAYOUT.sceneHeightCSS }}>
            <SceneContainer
              sceneLabel="3D drug lifecycle timeline visualization showing safety events and temporal patterns"
              cameraPosition={
          ...
          <div className="mt-golden-3 grid grid-cols-2 md:grid-cols-5 gap-golden-2">
            <div className="border border-sky-500/20 bg-sky-500/5 p-golden-1">
              <span className="text-[10px] font-mono uppercase text-sky-400/70">Events |V|</span>
              <p className="text-lg font-semibold text-white">{metrics.nodes}</p>
            </div>
            <div className="border border-sky-500/20 bg-sky-500/5 p-golden-1">
              <span className="text-[10px] font-mono uppercase text-sky-400/70">Temporal Links |E|</span>
              <p className="text-lg font-semibold text-white">{metrics.edges}</p>
            </div>
            <div className="border border-sky-500/20 bg-sky-500/5 p-golden-1">
              <span className="text-[10px] font-mono uppercase text-sky-400/70">Density</span>
              <p className="text-lg font-semibold text-white">{metrics.density.toFixed(3)}</p>
            </div>
            <div className="border border-sky-500/20 bg-sky-500/5 p-golden-1">
              <span className="text-[10px] font-mono uppercase text-sky-400/70">Avg Degree</span>
          ...
          {/* ── STEM Grounding ──────────────────────────────────────── */}
          {data.stem && (
            <div className="mt-golden-2 border border-sky-500/20 bg-sky-500/5 p-golden-2">
              <div className="flex items-center gap-golden-1 mb-golden-1">
                <span className="text-[10px] font-mono uppercase tracking-wider text-sky-400/60">STEM Grounding</span>
                <span className="text-[10px] font-mono text-sky-400/60">{data.stem.crate}</span>
              </div>
              ...
              <div className="mt-golden-1 flex items-center gap-golden-1 flex-wrap">
                ...
                {data.stem.tools.map(t => (
                  <code key={t} className="text-[10px] font-mono px-1.5 py-0.5 bg-nex-surface border border-nex-light text-sky-400/70">{t}</code>
                ))}
              </div>
            </div>
          )}
          ...
          {/* ── Selection Panel ─────────────────────────────────────── */}
          {selected && (
            <div className="mt-golden-2 p-golden-2 border border-sky-500/30 bg-sky-500/5">
              <div className="flex items-center gap-golden-2">
                <div className="w-3 h-3" style={{ backgroundColor: selected.color ?? '#0ea5e9' }} />
                ...
                <button
                  ...
                  className="ml-auto text-slate-dim/70 hover:text-white transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-400 px-1"
                >
                  <span aria-hidden="true">×</span>
                </button>
              </div>
              <p className="text-sm text-slate-dim/70 mt-golden-1">
                Temporal Links: {data.edges.filter(e => e.source === selected.id || e.target === selected.id).length} |
                Severity: {selected.value?.toFixed(2)}
              </p>
            </div>
          )}
          ...
          <div className="mt-golden-3 flex flex-wrap gap-golden-3 text-sm text-slate-dim/70">
            {TIMELINE_LEGEND.map(({ label, color }) => (
