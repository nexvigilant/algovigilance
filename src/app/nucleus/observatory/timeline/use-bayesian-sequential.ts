  ...
  const { data, error, isLoading } = useSWRData<BayesianResult>(
    ...
    async () => {
      const res = await fetch(
        "/api/nexcore/api/v1/mcp/pv_core_bayesian_sequential_beta_binomial",
        {
          method: "POST",
      ...
      if (!res.ok) throw new Error(`Bayesian MCP returned ${res.status}`);
      const json = (await res.json()) as McpBayesianResponse;
