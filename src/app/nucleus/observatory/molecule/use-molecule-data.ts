 ...
 *
 * When molecule.length >= 2, POSTs to /api/nexcore/api/v1/mcp/molecular_formula.
...
async function fetchMoleculeData(
  ...
  const res = await fetch("/api/nexcore/api/v1/mcp/molecular_formula", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ params: { formula: normalised } }),
  });

  if (!res.ok) throw new Error(`API returned ${res.status}`);
  const result: MolecularFormulaResult = await res.json();
