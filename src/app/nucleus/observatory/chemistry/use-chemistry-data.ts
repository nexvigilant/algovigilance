...
interface McpResponse {
  result?: HillResponseResult;
  data?: HillResponseResult;
}
...
async function fetchChemistry(compound: string): Promise<ObservatoryDataset> {
  const res = await fetch("/api/nexcore/api/v1/mcp/chemistry_hill_response", {
    method: "POST",
  ...
  if (!res.ok) {
    throw new Error(`Chemistry API returned ${res.status}`);
  }

  const json = (await res.json()) as McpResponse;
  const result: HillResponseResult | undefined = json.result ?? json.data;
  if (!result) {
    throw new Error("No data in chemistry API response");
  }
