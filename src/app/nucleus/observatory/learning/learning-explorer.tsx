...
import { LEARNING_STATUS_COLORS } from '@/components/observatory/observatory-constants'
...
export function LearningExplorer() {
  const { data, loading, error, refetch } = useLearningData()
  const [selected, setSelected] = useState<GraphNode | null>(null)
  const [graphLayout, setGraphLayout] = useState<GraphLayout>('hierarchy')
  ...
  const metrics = useMemo(() => {
    if (!data) return { nodes: 0, edges: 0, completed: 0, unlocked: 0, locked: 0 }
    const completed = data.nodes.filter(n => n.group === 'service').length
    const unlocked = data.nodes.filter(n => n.group === 'domain').length
    const locked = data.nodes.filter(n => n.group === 'foundation').length
    return {
      ...
      <header className="mb-golden-4">
        <div className="flex items-center gap-golden-2 mb-golden-2">
          ...
          <div className="flex h-8 w-8 shrink-0 items-center justify-center border border-emerald/30 bg-emerald/5">
            <GraduationCap className="h-4 w-4 text-emerald" aria-hidden="true" />
          </div>
          <div>
            <p className="text-[9px] font-mono uppercase tracking-[0.2em] text-emerald/70">{STRINGS.brandSubtitle}</p>
            <h1 className="text-sm font-semibold text-white">Learning Landscapes</h1>
          </div>
        </div>
      </header>

      <ExplorerNav current="learning" />

      {/* Loading / Error states */}
      {loading && (
        <div className="flex items-center gap-golden-2 mb-golden-3">
          <Loader2 className="h-4 w-4 text-emerald animate-spin" aria-hidden="true" />
          <span className="text-sm text-slate-dim/70">Loading learning pathway...</span>
        </div>
      )}
      {error && (
        <div role="alert" className="flex items-center gap-golden-2 mb-golden-3 p-golden-2 border border-red-500/30 bg-red-500/5">
          ...
          <button onClick={refetch} className="text-sm text-cyan hover:text-white transition-colors ml-auto focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400">
            Retry
          </button>
        </div>
      )}
      ...
      {data && (
        <>
          ...
          <div className="flex items-center gap-golden-1 mb-golden-2">
            ...
            {(['hierarchy', 'force', 'radial'] as const).map((mode) => (
              <button
                key={mode}
              ...
              >
                {mode === 'hierarchy' ? 'DAG Levels' : mode === 'force' ? 'Force' : 'Radial'}
              </button>
            ))}
          </div>
          ...
          <div style={{ height: LAYOUT.sceneHeightCSS }}>
            <SceneContainer
              sceneLabel="3D learning landscape visualization showing activities and prerequisites"
              cameraPosition={
                graphLayout === 'hierarchy' ? [0, 6, 20] :
          ...
          <div className="mt-golden-3 grid grid-cols-2 md:grid-cols-6 gap-golden-2">
            <div className="border border-emerald/20 bg-emerald/5 p-golden-1">
              <span className="text-[10px] font-mono uppercase text-emerald/70">Activities</span>
              <p className="text-lg font-semibold text-white">{metrics.nodes}</p>
            </div>
            <div className="border border-emerald/20 bg-emerald/5 p-golden-1">
              <span className="text-[10px] font-mono uppercase text-emerald/70">Prerequisites</span>
              <p className="text-lg font-semibold text-white">{metrics.edges}</p>
            </div>
            <div className="border border-emerald/20 bg-emerald/5 p-golden-1">
              <span className="text-[10px] font-mono uppercase text-emerald/70">Completed</span>
              <p className="text-lg font-semibold text-emerald">{metrics.completed}</p>
            </div>
            <div className="border border-cyan/20 bg-cyan/5 p-golden-1">
              <span className="text-[10px] font-mono uppercase text-cyan/70">Unlocked</span>
              <p className="text-lg font-semibold text-cyan">{metrics.unlocked}</p>
            </div>
            <div className="border border-nex-light bg-nex-surface p-golden-1">
              <span className="text-[10px] font-mono uppercase text-slate-dim/70">Locked</span>
              <p className="text-lg font-semibold text-slate-dim/70">{metrics.locked}</p>
            </div>
          ...
          {/* STEM Grounding */}
          {data.stem && (
            <div className="mt-golden-2 border border-emerald/20 bg-emerald/5 p-golden-2">
              <div className="flex items-center gap-golden-1 mb-golden-1">
                <span className="text-[10px] font-mono uppercase tracking-wider text-emerald/60">STEM Grounding</span>
                <span className="text-[10px] font-mono text-emerald/60">{data.stem.crate}</span>
              </div>
              ...
              <div className="mt-golden-1 flex items-center gap-golden-1 flex-wrap">
                ...
                {data.stem.tools.map(t => (
                  <code key={t} className="text-[10px] font-mono px-1.5 py-0.5 bg-nex-surface border border-nex-light text-emerald/70">{t}</code>
                ))}
              </div>
            </div>
          )}
          ...
          {/* Selection Panel */}
          {selected && (
            <div className="mt-golden-2 p-golden-2 border border-emerald/30 bg-emerald/5">
              <div className="flex items-center gap-golden-2">
                <div className="w-3 h-3" style={{ backgroundColor: selected.color ?? '#10b981' }} />
              ...
              <p className="text-sm text-slate-dim/70 mt-golden-1">
                Prerequisites: {data.edges.filter(e => e.target === selected.id).length} |
                Unlocks: {data.edges.filter(e => e.source === selected.id).length} |
          ...
          <div className="mt-golden-3 flex flex-wrap gap-golden-3 text-sm text-slate-dim/70">
            {LEARNING_STATUS_COLORS.map(({ label, color }) => (
