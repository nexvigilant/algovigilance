...
async function fetchEpidemiology(cohortName: string): Promise<EpiDataset> {
  const res = await fetch(
    "/api/nexcore/api/v1/mcp/epidemiology_relative_risk",
    {
      method: "POST",
  ...
  if (!res.ok) {
    throw new Error(`Epidemiology API returned ${res.status}`);
  }

  const json = (await res.json()) as McpEpiResponse;
  const result: EpiResult | undefined = json.result ?? json.data;
  if (!result) {
    throw new Error("No data in epidemiology API response");
  }
