...
async function fetchSurvival(
  ...
  // Fetch KM + log-rank in parallel
  const kmPromise = fetch("/api/nexcore/api/v1/mcp/pv_core_kaplan_meier", {
    method: "POST",
    ...
    .then((r) => (r.ok ? (r.json() as Promise<KmResponse>) : null))
    .catch(() => null);

  const logRankPromise =
    preset.groups.length > 1
      ? fetch("/api/nexcore/api/v1/mcp/pv_core_log_rank", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            times: allTimes,
          ...
          .then((r) => (r.ok ? (r.json() as Promise<LogRankResponse>) : null))
          .catch(() => null)
