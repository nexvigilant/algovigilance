...
import { CAREER_GROUP_COLORS } from '@/components/observatory/observatory-constants'
...
import { useCareerData } from './use-career-data'
...
export function CareerExplorer() {
  const { data, loading, error, refetch } = useCareerData()
    ...
    <div className="mx-auto max-w-7xl px-4 py-golden-4">
      ...
      <header className="mb-golden-4">
        <div className="flex items-center gap-golden-2 mb-golden-2">
          ...
          <div>
            <p className="text-[9px] font-mono uppercase tracking-[0.2em] text-gold/70">{STRINGS.brandSubtitle}</p>
            <h1 className="text-sm font-semibold text-white">Career Pathways</h1>
          </div>
        </div>
      </header>

      <ExplorerNav current="careers" />
      ...
      {loading && (
        <div className="flex items-center gap-golden-2 mb-golden-3">
          ...
          <span className="text-sm text-slate-dim/70">Loading career data...</span>
        </div>
      )}
      {error && (
        ...
        <>
          ...
          <div className="flex items-center gap-golden-1 mb-golden-2">
            ...
            {(['force', 'hierarchy', 'radial'] as const).map((mode) => (
              ...
              >
                {mode === 'force' ? 'Force' : mode === 'hierarchy' ? 'Salary Tiers' : 'Radial'}
              </button>
            ))}
          </div>
          ...
          <div style={{ height: LAYOUT.sceneHeightCSS }}>
            <SceneContainer
              sceneLabel="3D career pathway visualization showing roles and transitions"
              cameraPosition={
          ...
          <div className="mt-golden-3 grid grid-cols-2 md:grid-cols-5 gap-golden-2">
            <div className="border border-gold/20 bg-gold/5 p-golden-1">
              <span className="text-[10px] font-mono uppercase text-gold/70">Roles |V|</span>
              <p className="text-lg font-semibold text-white">{metrics.nodes}</p>
            </div>
            <div className="border border-gold/20 bg-gold/5 p-golden-1">
              <span className="text-[10px] font-mono uppercase text-gold/70">Transitions |E|</span>
              <p className="text-lg font-semibold text-white">{metrics.edges}</p>
            </div>
            <div className="border border-gold/20 bg-gold/5 p-golden-1">
              <span className="text-[10px] font-mono uppercase text-gold/70">Density</span>
              <p className="text-lg font-semibold text-white">{metrics.density.toFixed(3)}</p>
            </div>
            <div className="border border-gold/20 bg-gold/5 p-golden-1">
              <span className="text-[10px] font-mono uppercase text-gold/70">Avg Degree</span>
          ...
          {/* Selection Panel */}
          {selected && (
            <div className="mt-golden-2 p-golden-2 border border-gold/30 bg-gold/5">
              <div className="flex items-center gap-golden-2">
                <div className="w-3 h-3" style={{ backgroundColor: selected.color ?? '#eab308' }} />
              ...
              <p className="text-sm text-slate-dim/70 mt-golden-1">
                Transitions: {data.edges.filter(e => e.source === selected.id || e.target === selected.id).length} |
          ...
          <div className="mt-golden-3 flex flex-wrap gap-golden-3 text-sm text-slate-dim/70">
            {CAREER_GROUP_COLORS.map(({ label, color }) => (
